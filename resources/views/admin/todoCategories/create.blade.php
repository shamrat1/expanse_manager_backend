...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.todoCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.todo-categories.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                <label for="name">{{ trans('cruds.todoCategory.fields.name') }}*</label>
                <input type="text" required id="name" name="name" class="form-control" value="{{ old('name', isset($cat) ? $cat->task : '') }}">
                @if($errors->has('name'))
